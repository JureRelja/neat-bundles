...
Shopify.formatMoney = function (cents, format) {
    ...
    function formatWithDelimiters(number, precision, thousands, decimal) {
        ...
        let parts = number.split("."), dollars = parts[0].replace(/(\d)(?=(\d\d\d)+(?!\d))/g, "$1" + thousands), cents = parts[1] ? decimal + parts[1] : "";
    ...
    if (stepInputs.length == 0) {
        ...
        if (stepInput) {
            ...
            if (contentInputIndex != -1) {
                stepInput.inputs[contentInputIndex] = Object.assign(Object.assign({}, stepInput.inputs[contentInputIndex]), { value: event.target.files ? event.target.files[0] : event.target.value });
            }
