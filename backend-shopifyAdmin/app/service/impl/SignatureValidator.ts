...
export class SignatureValidator {
  ...
  public async verifySignature(): Promise<boolean> {
    ...
    const calculatedSignature = Hex.stringify(
      HmacSha256(
        queryParamsForSigning,
  ...
  private getUrlForSigning(): string {
    const queryParamEntries: URLSearchParamsIterator<[string, string]> =
      this.otherQueryParams.entries();

    let paramKeyValuePair: IteratorResult<[string, string], unknown> =
      queryParamEntries.next();
